...
def calibrate_risk(pred_risk, event_at_t_star, method="isotonic", random_state=42):
    """
    Applies post-hoc calibration to predicted risks using either isotonic or Platt scaling.

    Parameters:
        pred_risk (np.array): Raw predicted risks from the model
        event_at_t_star (np.array): Binary outcomes (1 = event before t*, 0 = no event)
        method (str): "isotonic" or "platt"
        random_state (int): for reproducibility of train/test split (optional)
    
    Returns:
        recalibrated_risk (np.array): calibrated version of pred_risk
    """
    # Split data for calibration fitting and evaluation to avoid overfitting
    X_train, X_test, y_train, y_test = train_test_split(
        pred_risk, event_at_t_star, test_size=0.2, random_state=random_state
    )

    if method == "isotonic":
        calibrator = IsotonicRegression(out_of_bounds='clip')
        calibrator.fit(X_train, y_train)
        recalibrated_risk = calibrator.transform(pred_risk)

    elif method == "platt":
        calibrator = LogisticRegression(solver='lbfgs')
        calibrator.fit(X_train.reshape(-1, 1), y_train)
        recalibrated_risk = calibrator.predict_proba(pred_risk.reshape(-1, 1))[:, 1]

    else:
        raise ValueError("Unsupported method. Use 'isotonic' or 'platt'.")

    return recalibrated_risk
...
for method_idx, folder in enumerate(calib_all_methods["folder_names"]):
    ...
    for csv_file in csv_files:
        ...
        df['pred_risk_raw'] = df['pred_risk']  # store original for reference
        df['pred_risk'] = calibrate_risk(df['pred_risk'].values, df['event_at_t_star'].values, method="isotonic")
