...
for i in range(10):
    ...
    if i==0:
        sns.scatterplot(x="time", y="normalized_output", data=merged_df, color="dodgerblue",edgecolor="black", alpha=0.5, label="Proposed", zorder=2)
    else:
        sns.scatterplot(x="time", y="normalized_output", data=merged_df, color="dodgerblue",edgecolor="black", alpha=0.5, zorder=2)
    ...
    max_value =  max(max(merged_df["time"]), max(merged_df["normalized_output"]))
    ...
    merged_df["normalized_output"] = (merged_df["output"] - merged_df["output"].min()) / (merged_df["output"].max() - merged_df["output"].min())
    ...
    sns.set_theme(style="ticks")
    if i==0:
        sns.scatterplot(x="time", y="normalized_output", data=merged_df, color="yellow",edgecolor="black", alpha=0.5, label="Cox w. CAC", zorder=1)
    else:
        sns.scatterplot(x="time", y="normalized_output", data=merged_df, color="yellow",edgecolor="black", alpha=0.5, zorder=1)
    ax = plt.gca() # Get a matplotlib's axes instance
    ...
    plt.savefig(os.path.join(csv_folder, 'calib_plot_full_fold_merged_%s.png' %i), bbox_inches='tight', dpi=300)
    plt.savefig(os.path.join(csv_folder, 'calib_plot_full_fold_merged_%s.eps' %i), bbox_inches='tight', dpi=300)
    plt.close()
