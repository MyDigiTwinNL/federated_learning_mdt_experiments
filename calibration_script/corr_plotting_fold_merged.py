...
for i in range(10):
    ...
    sns.set_theme(style="ticks")
    if i==0:
        sns.scatterplot(x="time", y="true_output_normalized", data=cox_true_df, color="yellow",edgecolor="black", alpha=0.5, label="Cox w. CAC", zorder=1)
    else:
    ...
    plt.savefig(os.path.join(csv_folder, 'calib_plot_fold_merged_%s.png' %i), bbox_inches='tight', dpi=500)
    plt.savefig(os.path.join(csv_folder, 'calib_plot_fold_merged_%s.eps' %i), bbox_inches='tight', dpi=500)
    plt.close()
