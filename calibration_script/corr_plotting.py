...
sns.scatterplot(x="time", y="true_output_normalized", data=nn_true_df)
...
plt.savefig(os.path.join(csv_folder, 'calib_plot.png' ), bbox_inches='tight', dpi=300)
plt.savefig(os.path.join(csv_folder, 'calib_plot.eps' ), bbox_inches='tight', dpi=300)
plt.close()
...
# matplotlib.use('Agg')
sns.set_theme(style="ticks")
sns.scatterplot(x="time", y="true_output_normalized", data=cox_true_df)
ax = plt.gca() # Get a matplotlib's axes instance
# plt.title("Agatston score correlation")
plt.xlabel("Time-to-event (Days)")
plt.ylabel("Normalized predictied partial hazard")
